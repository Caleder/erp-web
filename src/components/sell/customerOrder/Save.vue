<template>
  <div>
    ...
    <el-card>
      ...
      <el-row>
        <el-col :offset="20">
          <el-button @click="saveCustomerOrder">保存</el-button>
          <el-button type="primary"
                     @click="saveCustomerOrderThenNew">保存并新增</el-button>
        </el-col>
...
export default {
  ...
  created() {
    ...
    if (customerOrderId !== undefined) {
      ...
      this.getCustomerOrderDetail(customerOrderId)
    } else {
      this.getCustomerOrderCode()
    }
  ...
  methods: {
    ...
    // 获取单据编号
    async getCustomerOrderCode() {
      const { data: result } = await this.$http.post('/sell/customerOrder/createCode')
      if (!result.success) return this.$message.error(result.message)
      ...
      this.customerOrderCode = result.data.code
      this.saveForm.code = this.customerOrderCode
    },
    // 获取详情
    async getCustomerOrderDetail(customerOrderId) {
      const { data: result } = await this.$http.post('/sell/customerOrder/detail', {
        customerOrderId
      })
      if (!result.success) return this.$message.error(result.message)

      console.log(result.data)
      this.saveForm = result.data.customerOrder
    },
    // 点击按钮，保存客户订单
    async saveCustomerOrder() {
      this.$refs.saveFormRef.validate(async (valid) => {
        if (!valid) return
        // 可以发起新增客户订单的网络请求
        const { data: result } = await this.$http.post('/sell/customerOrder/save', {
          customerOrder: this.saveForm,
    ...
    // 保存并新增
    async saveCustomerOrderThenNew() {
