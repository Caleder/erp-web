  ...
  data () {
    return {
      ...
      saveFormRules: {
        ...
        vatRate: [
          { required: true, message: '请设置增值税税率', trigger: 'blur' }
        ]
      },
      // 是否新增联系人
      isContactAdd: true,
  ...
  methods: {
    ...
    async getSupplierPage () {
      ...
      this.supplierList.forEach((supplier) => {
        supplier.beginBalanceDisplay = (supplier.beginBalance / 100.0).toFixed(2)
      })
    ...
    async showEditDialog (supplierId) {
      ...
      supplier.beginReceivableAmountDisplay = (supplier.beginReceivableAmount / 100.0).toFixed(2)
      supplier.beginPrepaidAmountDisplay = (supplier.beginPrepaidAmount / 100.0).toFixed(2)
